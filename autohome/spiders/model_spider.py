...
import json
import scrapy
from autohome.items import ModelItem
from scrapy.spiders import CrawlSpider, Rule
from scrapy.linkextractors import LinkExtractor
from scrapy import log
from autohome.constants import levelMap
...
class ModelSpider(CrawlSpider):
    ...
    rules = (
        # 字母分区页
        Rule(LinkExtractor(allow=(r'http://www.autohome.com.cn/grade/carhtml/\S.html',)), callback='parse',
             follow=True),

        # 车系详情
        Rule(LinkExtractor(allow=(r'.*www\.autohome\.com\.cn/\d+.*',)), callback='parse_model_selling',
             follow=True),

        # 历史年款车型
        Rule(LinkExtractor(allow=(r'.*www\.autohome\.com\.cn/ashx/series_allspec\.ashx?s=\d+&y=\d+&l=\d+$',)),
    ...
    def parse_model_selling(self, response):
        ...
        # 判断此车系是否是在售车系
        stop_sell = response.xpath('/html/body').re(r'(指导价（停售）)')
        ...
        # 如果该车系是停售车系
            ...
            for model_tag in model_tags:
                ...
                price = model_tag.xpath('td[@class="price_d"]/text()').re(r'(\d+\.\d+)')
